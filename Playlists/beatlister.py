...
scrapped_data_file = "D:\\Met-PC\\Desktop\\beatlister\\combinedScrappedData.json" 

min_star = 9.0
max_star = 15.0
playlist_name = "9+ Star"

# File filtered by update_ranked_maps() to only include ranked maps.
ranked_data_file = "D:\\Met-PC\\Desktop\\beatlister\\combinedRankedData.json"

# File where the filtered and sorted playlist is outputted.
playlist_file = f"D:\\Met-PC\\Desktop\\beatlister\\{playlist_name}.json"
...
def get_ranked_maps():
	...
	for map_info in scrapped_data:
		for diff in map_info["Diffs"]:
			if diff["Ranked"]:
				ranked_maps.append(map_info)
				break

	return ranked_maps

# Filter ranked maps
ranked_maps = get_ranked_maps()

# Filter and sort maps by star rating
filtered_maps = []
for map_info in ranked_maps:
	for diff in map_info["Diffs"]:
		if not diff["Ranked"]: continue
